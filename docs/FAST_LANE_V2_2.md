...
This is the active dashboard-specific FAST-LANE v2.2 startup contract. The older versioned filename remains only for backward compatibility. The canonical cross-project policy is maintained in `rozkalnsandris/ops-workflows`; repository-local stricter security boundaries always win.
...
Merge remains an explicit owner decision. Merge never authorizes deployment, host/root/systemd/Docker/Cloudflare mutation, secrets, Quick Command/PTY activation, or another live write.
