...
Every HTTP response produced through the Fastify application receives the fixed policy below plus a fresh per-response CSP nonce on the `script-src` directive:

```text
Content-Security-Policy: default-src 'none'; base-uri 'none'; connect-src 'self' wss://dash.rozkalns.net; font-src 'self'; form-action 'self'; frame-ancestors 'none'; img-src 'self' data:; manifest-src 'self'; object-src 'none'; script-src 'self' 'nonce-<random>'; style-src 'self' 'unsafe-inline'; worker-src 'self'
X-Content-Type-Options: nosniff
Referrer-Policy: no-referrer
Permissions-Policy: camera=(), geolocation=(), microphone=(), payment=(), usb=()
```

The nonce is generated with cryptographic randomness for every response. It exists so Cloudflare JavaScript Detections can attach the origin nonce to its injected inline script without weakening the application's script policy. The application does not add `unsafe-inline` or `unsafe-eval` to `script-src`.
...
- `script-src 'self' 'nonce-<random>'` allows bundled first-party JavaScript and only inline script carrying the unpredictable per-response nonce. There is no `unsafe-inline` and no `unsafe-eval` script exception.
...
- `manifest-src 'self'`, `font-src 'self'` and `img-src 'self' data:` cover current first-party application resources. The PWA manifest link uses credentialed CORS mode; CSP still does not trust a cross-origin manifest source.
...
- every response receives a fresh cryptographically random script nonce;
...
- CSP keeps scripts on `'self'`, contains no `unsafe-eval`, and does not add `unsafe-inline` to the script directive;
