...
import { isDirectCliInvocation } from "./docker-broker-entry.js";
...
describe("docker broker CLI entrypoint detection", () => {
  it("accepts the systemd current-symlink path for the real release module", async () => {
    const root = await mkdtemp(join(tmpdir(), "dashboard-rpi5-broker-entry-"));
    temporaryRoots.push(root);

    const release = join(root, "releases", "target");
    await mkdir(release, { recursive: true });
    const modulePath = join(release, "docker-broker-entry.js");
    await writeFile(modulePath, "// fixture\n", "utf8");

    const current = join(root, "current");
    await symlink(release, current, "dir");
    const invokedPath = join(current, "docker-broker-entry.js");
  ...
  it("rejects imports, unrelated executables and missing paths", async () => {
    ...
    const modulePath = join(root, "docker-broker-entry.js");
