...
import { DEFAULT_LOG_BROKER_SOCKET_PATH, LOG_BROKER_SOCKET_ENV } from "./log-broker-protocol.js";
...
interface StartLogBrokerOptions { socketPath?: string; reader?: PrivilegedLogReader; }
...
async function runFromCli() {
  ...
  let closing = false;
  const close = () => { if (closing) return; closing = true; running.server.close(() => undefined); };
