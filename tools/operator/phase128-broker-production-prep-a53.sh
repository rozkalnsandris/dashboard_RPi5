...
blocked() {
  echo "PHASE128_A53_PREP_BLOCKED: $*" >&2
  exit 1
}

trap 'rc=$?; if [ "$rc" -ne 0 ]; then echo "PHASE128_A53_PREP_EXIT=$rc PRODUCTION_MUTATION=NO RELEASE_APPLY=NO SYSTEMD_MUTATION=NO IDENTITY_MUTATION=NO CLOUDFLARE_MUTATION=NO AUTO_RETRY=NO AUTO_CLEANUP=NO" >&2; fi' EXIT
...
[ "${#OLD_INVALID_GROUP}" -gt 32 ] || blocked "regression fixture no longer proves invalid old group length"

printf 'PHASE128_A53_PREP_START target=%s expected_current=%s broker_client_group=%s\n' \
  "$TARGET" "$EXPECTED_CURRENT" "$BROKER_CLIENT_GROUP"

# ---------------------------------------------------------------------------
# 1. Authoritative GitHub main + exact push->main CI.
# ---------------------------------------------------------------------------
main_json="$(curl -fsSL \
...
# ---------------------------------------------------------------------------
# 2. Fresh production read-only preflight. No protected agent-socket traversal
#    is attempted as the operator; socket probes run only as dashboard web.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. Prepare exact main only in an operator-owned cache workspace.
#    Never reuse or delete an existing workspace automatically.
# ---------------------------------------------------------------------------
...
grep -qF 'toBeLessThanOrEqual(32)' "$systemd_test" || blocked "systemd account-name bound regression missing"
...
# ---------------------------------------------------------------------------
# 4. Deterministic validation/build from exact main.
# ---------------------------------------------------------------------------
(
  cd "$REPO"
...
# ---------------------------------------------------------------------------
# 5. Generate, digest-bind and fully verify immutable candidate.
# ---------------------------------------------------------------------------
...
printf 'CANDIDATE_VERIFY_PASS sha256=%s files=%s bytes=%s manifest_sha256=%s broker_unit_sha256=%s agent_unit_sha256=%s broker_entry_sha256=%s\n' \
  "$candidate" "$files" "$bytes" "$manifest_sha" "$manifest_broker_unit_sha" "$manifest_agent_unit_sha" "$manifest_broker_entry_sha"
...
# ---------------------------------------------------------------------------
# 7. Canonical release-controller PLAN only. This helper contains no --apply.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8. Re-prove that preparation changed no production/trust boundary state.
# ---------------------------------------------------------------------------
...
getent group "$BROKER_CLIENT_GROUP" >/dev/null && blocked "broker group appeared during prep"
...
printf 'PHASE128_A53_PREPARATION_READY target=%s ci_run=%s attempt=%s run_id=%s candidate=%s files=%s bytes=%s current=%s agent_pid=%s web_pid=%s broker_client_group=%s\n' \
  "$TARGET" "$run_number" "$run_attempt" "$run_id" "$candidate" "$files" "$bytes" "$EXPECTED_CURRENT" "$agent_pid" "$web_pid" "$BROKER_CLIENT_GROUP"
printf 'PHASE128_A53_STOP production_mutation=NO release_apply=NO systemd_mutation=NO identity_mutation=NO docker_group_main_agent=NO cloudflare=UNCHANGED quick=404 terminal=absent\n'
