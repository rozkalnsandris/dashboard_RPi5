...
blocked() {
  echo "ISSUE127_CANDIDATE_PREP_BLOCKED: $*" >&2
  exit 1
}

trap 'rc=$?; if [ "$rc" -ne 0 ]; then echo "ISSUE127_CANDIDATE_PREP_EXIT=$rc PRODUCTION_MUTATION=NO RELEASE_APPLY=NO SYSTEMD_MUTATION=NO IDENTITY_MUTATION=NO PERMISSION_MUTATION=NO CLOUDFLARE_MUTATION=NO SERVICE_RESTART=NO AUTO_RETRY=NO AUTO_CLEANUP=NO" >&2; fi' EXIT

need() {
  command -v "$1" >/dev/null 2>&1 || blocked "missing command: $1"
}

for command_name in curl jq git node npm sha256sum systemctl readlink stat id getent grep awk sed sudo tail tr find sort xargs cmp; do
  need "$command_name"
done
...
printf 'ISSUE127_CANDIDATE_PREP_START target=%s current=%s workspace=%s\n' \
...
###############################################################################
# 1. Exact GitHub main and push-to-main CI. This is deliberately discovered
#    by exact head SHA instead of assuming a run number.
###############################################################################
...
[ "$ci_status" = completed ] || blocked "exact-main CI not completed: status=$ci_status"
[ "$ci_conclusion" = success ] || blocked "exact-main CI not successful: conclusion=$ci_conclusion"

jobs_json="$(curl -fsSL \
  -H 'Accept: application/vnd.github+json' \
  -H 'X-GitHub-Api-Version: 2022-11-28' \
  "https://api.github.com/repos/$REPO_SLUG/actions/runs/$run_id/jobs?per_page=100")" \
  || blocked "exact-main CI jobs lookup failed"
...
for job_name in "check" "terminal-native (x64)" "terminal-native (arm64)"; do
  ...
  ')"
  [ "$count" -eq 1 ] || blocked "required exact-main CI job not success: $job_name count=$count"
done

printf 'ISSUE127_EXACT_MAIN_CI_PASS main=%s ci_run=%s attempt=%s run_id=%s\n' \
  "$TARGET" "$run_number" "$run_attempt" "$run_id"

###############################################################################
# 2. Read-only proof of the current production baseline after #145.
###############################################################################

current="$(readlink "$PROD_ROOT/current")" || blocked "current pointer unreadable"
[ "$current" = "releases/$EXPECTED_CURRENT" ] \
  || blocked "current pointer drift expected=releases/$EXPECTED_CURRENT actual=$current"
...
[ "$(sudo jq -er '.candidateSha256' "$CURRENT_MANIFEST")" = "$EXPECTED_CURRENT_CANDIDATE" ] \
  || blocked "current candidate digest drift"
...
[ "$broker_cwd" = "$PROD_ROOT/releases/$EXPECTED_BROKER_RELEASE" ] \
  || blocked "broker cwd drift: $broker_cwd"
...
[ "$(sudo stat -Lc '%U:%G:%a:%F' "$QUICK_DROPIN")" = 'root:root:644:regular file' ] \
  || blocked "Quick Commands drop-in metadata drift"

agent_env="$(systemctl show "$AGENT_SERVICE" -p Environment --value)"
printf '%s\n' "$agent_env" | grep -q 'DASHBOARD_RPI5_QUICK_COMMANDS=enabled' \
  || blocked "Quick Commands effective environment not enabled"
...
[[ "$broker_gid" =~ ^[0-9]+$ ]] || blocked "broker-client GID unavailable"
[[ "$docker_gid" =~ ^[0-9]+$ ]] || blocked "Docker GID unavailable"
[[ "$video_gid" =~ ^[0-9]+$ ]] || blocked "video GID unavailable"
...
[ "$(sudo stat -Lc '%U:%G:%a:%F' /var/run/docker.sock)" = 'root:docker:660:socket' ] \
  || blocked "Docker socket metadata drift"
...
# Old production broker must not already expose the new #127 routes.
for broker_log_path in \
  '/v1/docker/logs/homeassistant/15m' \
  '/v1/docker/logs/prometheus/24h'; do
...
printf 'ISSUE127_PRODUCTION_PREFLIGHT_PASS current=%s broker_pid=%s agent_pid=%s web_pid=%s host=200 docker=200 logs=503 events=503 quick=200 terminal=absent access=302\n' \
  "$EXPECTED_CURRENT" "$broker_pid" "$agent_pid" "$web_pid"

###############################################################################
# 3. Fresh operator-owned cache workspace only. No reuse or cleanup.
###############################################################################
...
###############################################################################
# 4. Pin the merged #127 trust-boundary contract before building.
###############################################################################
...
grep -qF 'export const DOCKER_BROKER_LOG_MAX_RESPONSE_BYTES = 512 * 1024;' "$protocol" \
  || blocked "Docker log response-byte bound missing"
grep -qF 'export const DOCKER_BROKER_LOG_TAIL = 400;' "$protocol" \
  || blocked "Docker log tail bound missing"
grep -qF 'export const DOCKER_BROKER_LOG_SOURCES = ["homeassistant", "prometheus"] as const;' "$protocol" \
  || blocked "Docker log source allowlist drift"
grep -qF 'export const DOCKER_BROKER_LOG_RANGES = ["15m", "1h", "6h", "24h"] as const;' "$protocol" \
  || blocked "Docker log range allowlist drift"

grep -qF 'homeassistant: "homeassistant"' "$broker_server" \
  || blocked "Home Assistant server-side target mapping missing"
grep -qF 'prometheus: "prometheus"' "$broker_server" \
  || blocked "Prometheus server-side target mapping missing"
grep -qF 'logs?stdout=true&stderr=true&since=${sinceSeconds}&timestamps=true&tail=${DOCKER_BROKER_LOG_TAIL}' "$broker_server" \
  || blocked "broker-owned fixed Docker log query contract missing"

grep -qF '"docker:homeassistant": { brokerSource: "homeassistant", containerName: "homeassistant" }' "$live_logs" \
  || blocked "live Home Assistant source mapping missing"
grep -qF '"docker:prometheus": { brokerSource: "prometheus", containerName: "prometheus" }' "$live_logs" \
  || blocked "live Prometheus source mapping missing"

grep -qx 'Environment=DASHBOARD_RPI5_QUICK_COMMANDS=disabled' "$agent_unit" \
  || blocked "base unit Quick Commands contract drift"

printf 'ISSUE127_SOURCE_CONTRACT_PASS sources=2 ranges=4 tail=400 max_bytes=524288 quick_base_unit=disabled\n'

###############################################################################
# 5. Deterministic exact-main validation and production build.
###############################################################################
...
###############################################################################
# 6. Generate and fully verify a fresh immutable candidate.
###############################################################################
...
node "$REPO/tools/production-candidate-manifest.mjs" \
  ...
  --sha "$TARGET" \
  > "$MANIFEST" \
  || blocked "candidate manifest generation failed"
...
node "$REPO/tools/production-candidate-manifest.mjs" \
  ...
  --verify "$MANIFEST" \
  | grep -q '"status":"PASS"' \
  || blocked "candidate manifest verification failed"
...
printf 'ISSUE127_CANDIDATE_VERIFY_PASS sha256=%s files=%s bytes=%s manifest_sha256=%s broker_entry_sha256=%s agent_entry_sha256=%s server_dist_sha256=%s\n' \
  "$candidate" "$files" "$bytes" "$manifest_sha" \
  "$manifest_broker_entry_sha" "$manifest_agent_entry_sha" "$server_dist_sha"

###############################################################################
# 7. Manifest-only isolated runtime smoke and release-controller PLAN only.
###############################################################################
...
printf 'ISSUE127_RUNTIME_SMOKE_PASS candidate=%s terminal=disabled\n' "$candidate"

plan="$(cd "$REPO" && sudo /usr/bin/node tools/production-release-controller.mjs \
  --candidate-root "$REPO" \
  --manifest "$MANIFEST" \
  --sha "$TARGET")" \
  || blocked "release-controller PLAN failed"
...
[ "$(printf '%s' "$plan" | jq -c '.operations')" = '["copy_manifest_allowlisted_release","write_verified_manifest_marker","atomic_current_symlink_swap"]' ] \
  || blocked "release PLAN operations mismatch"
printf 'ISSUE127_RELEASE_PLAN_PASS %s\n' "$plan"

###############################################################################
# 8. Re-prove there was no production/trust-boundary change during preparation.
###############################################################################
...
[ "$(response_status "$post_agent_docker")" = 200 ] || blocked "Docker current-state changed during prep"
for source_id in 'docker%3Ahomeassistant' 'docker%3Aprometheus'; do
...
printf 'ISSUE127_CANDIDATE_PREPARATION_READY target=%s ci_run=%s attempt=%s run_id=%s candidate=%s files=%s bytes=%s current=%s broker_pid=%s agent_pid=%s web_pid=%s workspace=%s\n' \
  "$TARGET" "$run_number" "$run_attempt" "$run_id" "$candidate" "$files" "$bytes" \
  "$EXPECTED_CURRENT" "$broker_pid" "$agent_pid" "$web_pid" "$WORKSPACE"
printf 'ISSUE127_CANDIDATE_PREP_STOP production_mutation=NO release_apply=NO systemd_mutation=NO identity_mutation=NO permission_mutation=NO main_agent_docker_group=NO main_agent_video_group=NO broker_restart=NO agent_restart=NO web_restart=NO cloudflare=UNCHANGED quick=200 events=503 docker_logs=503 terminal=absent\n'
